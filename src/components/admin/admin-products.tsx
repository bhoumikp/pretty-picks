...
import { buildFieldErrors, focusFirstInvalid, validateMinLength, validateNumberMin, validateRequired } from "@/lib/validation";
...
export default function AdminProductForm({
	...
	const categoryMissing = useMemo(
		() => Boolean(form.categoryId && !categories.some((category) => category.id === form.categoryId)),
	...
	const categoryOptions = useMemo(() => {
		...
		if (categoryMissing && form.categoryId) {
			return [
				{ value: form.categoryId, label: "Deleted category (removed)", disabled: true },
	...
	}, [categories, categoryMissing, form.categoryId]);
	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		const categoryError = !form.categoryId
			...
			: categoryMissing
			? { field: "Category", message: "Selected category was deleted. Please choose another." }
			: null;
		...
		>([
			...
			{
				key: "categoryId",
				error: categoryError ?? validateRequired(form.categoryId, "Category"),
				message: categoryError?.message ?? "Please select a category.",
			},
		...
		if (Object.keys(nextErrors).length > 0) {
			...
			focusFirstInvalid(nextErrors, [
				{ key: "name", selector: "#admin-product-name" },
				{ key: "categoryId", selector: "#admin-product-category" },
				{ key: "material", selector: "#admin-product-material" },
				{ key: "price", selector: "#admin-product-price" },
				{ key: "stock", selector: "#admin-product-stock" },
				{ key: "description", selector: "#admin-product-description" },
				{ key: "images", selector: "#admin-product-images" },
			]);
			setLoading(false);
			return;
		}
		...
		try {
			...
			setToasts((prev) => [
				...prev,
				{ id, type: "error", message: "Network error. Please try again." },
			]);
			setLoading(false);
		}
	};
					...
					<div className="grid items-start gap-4 md:grid-cols-2">
						...
						<div className="grid gap-2">
							...
							<AdminSelect
								...
								fullWidth
								triggerId="admin-product-category"
								buttonClassName={`w-full admin-input ${fieldErrors.categoryId ? "is-error" : ""}`}
								header="Category"
								ariaLabel="Category"
							/>
							{!fieldErrors.categoryId && categoryMissing && (
								<span className="text-xs normal-case text-amber-600">
