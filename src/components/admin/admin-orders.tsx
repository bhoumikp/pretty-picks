...
export default function AdminOrders({
	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (Object.keys(nextErrors).length > 0) {
			...
			focusFirstInvalid(nextErrors, [
				{ key: "productId", selector: "#admin-order-product" },
				{ key: "customerName", selector: "#admin-order-customer" },
				{ key: "phone", selector: "#admin-order-phone" },
			]);
			setLoading(false);
			return;
		}
	...
	if (variant === "bare") {
		return <>{formMarkup}{toastMarkup}</>;
	}

	return <div className="soft-card p-6">{formMarkup}{toastMarkup}</div>;
}
