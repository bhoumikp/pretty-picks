...
export default function AdminSubcategories({
	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (Object.keys(nextErrors).length > 0) {
			...
			focusFirstInvalid(nextErrors, [
				{ key: "parentId", selector: "#admin-subcategory-parent" },
				{ key: "name", selector: "#admin-subcategory-name" },
				{ key: "image", selector: "#admin-subcategory-image" },
			]);
			setSaving(false);
			return;
		}
	...
	const confirmDelete = async () => {
		...
		if (!response.ok) {
			let message = "Unable to delete sub category.";
			try {
				const data = (await response.json()) as { productCount?: number; error?: string };
				if (response.status === 409 && typeof data.productCount === "number") {
					message = `Sub category has ${data.productCount} product${data.productCount === 1 ? "" : "s"}. Remove them first.`;
				} else if (data.error) {
					message = data.error;
				}
			} catch {
	...
	const bulkArchive = async () => {
		...
		if (blocked.length > 0) {
			...
			setToasts((prev) => [
				...prev,
				{ id: toastId, type: "error", message: "Some sub categories could not be deleted." },
			]);
		}
		await fetchSubcategories(query, page, sort, dir, status, { force: true });
				...
				<div className="flex flex-wrap items-center justify-between gap-3">
					<div>
						...
						<div className="flex items-center gap-2">
							...
							<button
								type="button"
								onClick={() => fetchSubcategories(query, page, sort, dir, status, { force: true })}
						...
						>
							...
							<div className="grid gap-2">
								...
								<AdminSelect
									...
									fullWidth
									triggerId="admin-subcategory-parent"
							...
							<div className="grid gap-2">
								...
								<div className="flex flex-wrap items-center gap-3">
									...
									<CloudinaryUploadWidget
										maxFiles={1}
										onUpload={(uploads: Array<{ url: string }>) => {
											if (uploads.length > 0) {
									...
									{form.image ? (
										<div className="group relative aspect-[4/5] w-32 overflow-hidden rounded-lg border border-[var(--pp-border)] bg-[var(--pp-beige)]/40">
											<Image
												...
												onClick={() => setPreviewItem({ url: form.image, title: form.name || "Sub Category Image" })}
											/>
											<button
												type="button"
						...
						<div className="w-full max-w-5xl bg-white rounded-lg shadow-lg" onClick={(e) => e.stopPropagation()}>
							...
							<div className="max-h-[60vh] overflow-y-auto p-6">
								{libraryLoading ? (
									...
									<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
										{libraryItems.map((item: { id: string; url: string }) => {
							...
							<div className="flex items-center justify-between border-t border-[var(--pp-border)] bg-[var(--pp-beige)]/40 px-6 py-4">
								...
								<div className="flex gap-3">
									...
									<button
										...
										onClick={() => {
											const item = libraryItems.find((i: { id: string; url: string }) => i.id === selectedLibraryId);
											if (item) {
