...
interface AdminCategoriesTableProps {
	...
	onReorder?: (nextCategories: CategoryRow[]) => void;
...
function AdminCategoriesTable({
	...
	const handleDragOver = (event: React.DragEvent, index: number) => {
		...
		const item = nextCategories.splice(draggedIndex, 1)[0];
		nextCategories.splice(index, 0, item);
		
		setDraggedIndex(index);
		if (onReorder) onReorder(nextCategories);
	};
